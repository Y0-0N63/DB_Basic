/* VIEW : 논리적 가상 테이블 > 테이블 모양을 하고는 있지만, 실제로 값을 저장하고 있진 않음.
...
-- ORA-01031: 권한이 불충분합니다 > 관리자(sys)계정으로 접근해야
-- 관리자(SYS) 계정으로 접속하기 > VIEW 생성 권한 부여 (sys -> kh_cby 계정으로)
GRANT CREATE VIEW TO kh_cby;
-- 다시 kh_cby 계정으로 돌아와 VIEW 생성 구문 실행하기
CREATE VIEW V_EMP AS SELECT * FROM EMPLOYEE; -- 실행 성공
SELECT * FROM V_EMP; -- Tables가 아닌 Views에서 확인 가능

-- 사번, 이름, 부서명, 직급명 조회하기 위한 VIEW 생성
CREATE OR REPLACE VIEW V_EMP AS
SELECT EMP_ID 사번, EMP_NAME 이름, NVL(DEPT_TITLE, '없음') 부서명, JOB_NAME 직급명 FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
ORDER BY 사번;
-- ORA-00955: 기존의 객체가 이름을 사용하고 있습니다. > OR REPLACE 사용하기

-- 직급명이 대리인 사원 모두 조회 > VIEW 조회 결과로 보이는 컬럼명 사용해야
...
CREATE OR REPLACE VIEW V_DCOPY2 AS SELECT DEPT_ID, LOCATION_ID FROM DEPT_COPY2;
...
-- V_DCOPY2 VIEW에 INSERT수행하기
INSERT INTO V_DCOPY2 VALUES ('D0', 'L2');

SELECT * FROM V_DCOPY2; --> 가상 테이블 VIEW에 INSERT 성공
SELECT * FROM DEPT_COPY2; --> 원본 테이블에도 DO, L2가 추가되어 있음(DO, NULL, L2)
-- VIEW에 INSERT 수행했지만, 원본 테이블 DEPT_COPY2에도 값이 INSERT됨
-- VIEW를 생성할 때 사용된 컬럼에만 데이터 삽입되고, 그렇지 않은 컬럼(DEPT_TITLE)에는 NULL이 들어가있음
-- > NULL은 DB의 무결성을 약하게 만드는 주요 원인이므로 의도되지 않는 NULL은 존재하지 않게 해야 함
-- + 무결성 : DB에서 데이터를 정확하고 일관되게 유지하기 하게 위한 개념 (데이터의 정확성, 일관성, 신뢰성 보장)

-- WITH READ ONLY 옵션 사용하기
-- VIEW를 이용해 DML(INSERT/UPDATE/DELETE) 수행하는 것을 막기 위해 사용함
CREATE OR REPLACE VIEW V_DCOPY2 AS SELECT DEPT_ID, LOCATION_ID FROM DEPT_COPY2 WITH READ ONLY;
INSERT INTO V_DCOPY2 VALUES ('D0', 'L2'); -- ORA-42399: 읽기 전용 뷰에서는 DML 작업을 수행할 수 없습니다.
SELECT * FROM V_DCOPY2;
 ...
 * PK가 지정된 컬럼에 삽입될 값을 생성할 때 SEQUENCE를 이용하면 좋다! (학번, 사번...)
 * 
 *   [작성법]
  CREATE SEQUENCE 시퀀스이름
  [STRAT WITH 숫자] -- 처음 발생시킬 시작값 지정, 생략하면 1이 기본
  [INCREMENT BY 숫자] -- 다음 값에 대한 증가치, 생략하면 1이 기본
  [MAXVALUE 숫자 | NOMAXVALUE] -- 발생시킬 최대값 지정, 생략하면 기본값은 10^27 - 1 (즉, 매우 큰 값) / NOMAXVALUE를 사용하면 최대값 제한이 없음을 의미
...
--> CURRVAL : 가장 최근 호출된 NEXTVAL 값을 반환함을 의미 > NEXTVAL을 호출한 적 없어서 오류가 발생함 > NEXTVAL 호출하기

SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL;
SELECT SEQ_TEST_NO.CURRVAL FROM DUAL; -- 시퀀스 생성 후 첫 NEXTVAL = START WITH 값인 100

-- NEXTVAL 호출할 때마다 INCREMENT BY에 작성된 수만큼 증가하는지 확인
...
-- TB_TEST 테이블에 PK로 지정된 컬럼 값에 SEQ_TEST_NO 시퀀스로 생성하기
...
SELECT * FROM TB_TEST; -- 125 짱구, 130 철수, 135 유리

-- UPDATE에서 시퀀스 사용하기
-- '짱구'의 PK 컬럼값을 SEQ_TEST_NO 시퀀스의 다음 생성값으로 변경하기
UPDATE TB_TEST SET TEST_NO = SEQ_TEST_NO.NEXTVAL WHERE TEST_NAME = '짱구';
-- 네 번 수행 > 155? :  시퀀스 SEQ_TEST_NO.NEXTVAL exceeds MAXVALUE은 사례로 될 수 없습니다
--> MAXVALUE를 150으로 설정했기 때문

SELECT * FROM TB_TEST; -- 150 짱구 

-- SEQUENCE 변경 (ALTER) > START WITH 빼고 모두 가능
-- [작성법]
-- ALTER SEQUENCE 시퀀스이름
-- [INCREMENT BY 숫자]
-- [MAXVALUE 숫자 | NOMAXVALUE]
-- [MINVALUE 숫자 | MINVALUE]
-- [CYCLE | NOCYCLE]
-- [CACHE 바이트크기 | NOCACHE]

-- SEQ_TEST_NO의 MAXVALUE 값 변경하기
 ...
 * 
 * ** 인덱스의 단점 **
 * - 데이터 변경(INSERT,UPDATE,DELETE) 작업 시 이진 트리 구조에 변형이 일어남 -> DML 작업이 빈번한 경우 시스템 부하가 늘어 성능이 저하됨.
...
-- ROWID와 INDEX의 관계성
--> INDEX는 컬럼의 값과 해당 행의 ROWID를 매핑하여 ROWID의 값을 저장함 > INDEX의 핵심 역할은 컬럼값을 해당 행의 ROWID를 통해 빠르게 찾아내는 것!
...
-- 조건문(IF/CASE), 반복문(LOOP, WHILE), 변수 선언 가능
BEGIN 
	FOR I IN 1..1000000 -- I는 1~100만 반복
	LOOP
...
-- 인덱스를 사용해서 검색하는 방법 > WHERE 절에 INDEX가 지정된 컬럼을 언급하기
-- 인덱스 사용하지 않고 TEST_ID 'TEST500000'인 행 조회하기
SELECT * FROM TB_IDX_TEST WHERE TEST_ID = 'TEST500000';

-- 인덱스 사용하고 TEST_NO가 500000인 행 조회하기
