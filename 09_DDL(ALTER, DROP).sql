...
SELECT * FROM DEPT_COPY; -- 컬럼이 생성되며 DEFAULT 값 자동 삽입되었음을 확인

-- D10 개발1팀 추가
INSERT INTO DEPT_COPY VALUES ('D10', '개발1팀', 'L1', DEFAULT, DEFAULT);
-- ORA-12899: "KH_CBY"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
-- DEPT_ID(CHAR(2)) > 영어, 숫자 총 2글자까지 저장 가능하나 D10은 CHAR(3)
--> VARCHAR2(3)으로 변경하기 (남는 바이트 메모리 반환을 위해 VARCHAR 사용)

-- DEPT_ID 컬럼 데이터 타입 수정
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR2(3);
INSERT INTO DEPT_COPY VALUES ('D10', '개발1팀', 'L1', DEFAULT, DEFAULT);
SELECT * FROM DEPT_COPY; -- 컬럼의 데이터 타입 변경 후 > 삽입 성공

-- LNAME의 기본값을 'KOREA'로 수정
ALTER TABLE DEPT_COPY MODIFY LNAME DEFAULT 'KOREA';
--> 기본값을 변경했다고 해서 기존 데이터가 변하지는 않음 > 새로 삽입될 행에서부터 변경된 기본값 적용
...
-- TB1과 TB2는 부모-자식 테이블 관계
-- TB1(부모) 테이블을 삭제하려고 할 때
DROP TABLE TB1;
-- ORA-02449: 외래 키에 의해 참조되는 고유/기본 키가 테이블에 있습니다
--> 1) 자식, 부모 테이블 순서로 삭제
--> 2) ALTER를 이용해 FK 제약조건 삭제 후 TB1 삭제
--> 3) DROP TABLE 삭제 옵션 중 CASCADE CONSTRAINTS(삭제하려는 테이블과 연결된 FK 제약조건 모두 삭제) 사용 

DROP TABLE TB1 CASCADE CONSTRAINTS;
SELECT * FROM TB1; -- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
SELECT * FROM TB2; -- TB2 테이블은 독립적인(타 테이블과 관계를 맺지 않는) 테이블로 혼자 남게 됨

-- DDL 주의사항
-- 1) DDL은 COMMIT과 ROLLBACK의 대상이 아님 > DDL과 DML 구문을 섞어서 수행하면 안 됨!
--> DDL은 수행 시 존재하고 있는 트랜잭션을 모두 DB에 강제 COMMIT함 > DDL이 전부 종료된 후 DML 구문을 수행하는 것이 권장됨
-- DDL (CREATE, ALTER, DROP) : **객체** 생성, 수정, 삭제
-- DML (INSERT, UDPATE, DELETE) : **데이터(행)** 추가, 갱신, 삭제
...
SELECT * FROM TB2; -- TRANSACTION에 포함, DB에 영구 저장 상태는 아님
...
SELECT * FROM TB2; -- 14, 15 모두 조회됨 > ROLLBACK되지 않음
-- DDL 구문(ALTER)를 사용하여 그 시점에 COMMIT됨