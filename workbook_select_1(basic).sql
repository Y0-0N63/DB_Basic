-- Basic SELECT
-- 1. 춘 기술대학교의 학과 이름과 계열을 표시하시오. 단, 출력 해더는 "학과 명", "계열"으로 표시하도록 한다.
...
AND
SUBSTR(STUDENT_SSN, 8, 1) = '2' -- 여학생 조회
AND
ABSENCE_YN = 'Y'; -- 현재 휴학 여부 조회

-- 4. 도서관에서 대출 도서 장기 연체자들을 찾아 이름을 게시하고자 한다. 대상자들의 학번이 아래와 같을 때 대상자를 찾는 SQL 구문 작성
-- 대상자들의 학번 : A513079, A513090, A513091, A513110, A513119
SELECT STUDENT_NAME FROM TB_STUDENT 
...
-- 6. 춘 기술대학교는 총장을 제외하고 모든 교수들이 소속 학과를 가지고 있다. 그럼 춘 기술대학교 총장의 이름을 알아낼 수 있는 SQL 구문 작성
...
-- 8. 수강신청을 하려고 한다. 선수 과목이 존재하는 과목의 번호를 조회하시오.
SELECT CLASS_NO FROM TB_CLASS WHERE PREATTENDING_CLASS_NO IS NOT NULL;

-- 9. 춘 대학에는 어떤 계열(CATEGORY)들이 있는지 조히해보시오.
...
WHERE TO_CHAR(ENTRANCE_DATE, 'YY') = '02' -- 02 학번 조회
AND SUBSTR(STUDENT_ADDRESS, 1, 2) = '전주' -- 전주 거주자 조회
AND ABSENCE_YN = 'N'; -- 휴학한 사람 제외