...
-- 1. 현재 주문 내역 중 아직 처리되지 않은 주문을 조회하시오. (고객명, 주문일, 처리상태)
SELECT NAME 고객명, ORDER_DATE 주문일, STATUS 처리상태
FROM ORDERS JOIN CUSTOMERS USING (CUSTOMER_ID)
WHERE STATUS = 'N';

-- 2. '홍길동' 고객이 2024년도에 주문한 전체 내역(주문번호, 주문날짜, 처리상태)을 조회하시오.
SELECT CUSTOMER_ID FROM CUSTOMERS WHERE NAME = '홍길동';
SELECT ORDER_DATE FROM ORDERS WHERE EXTRACT(YEAR FROM ORDER_DATE) = 2024;

SELECT ORDER_ID 주문번호, ORDER_DATE 주문날짜, STATUS 처리상태 FROM ORDERS
WHERE CUSTOMER_ID = (SELECT CUSTOMER_ID FROM CUSTOMERS WHERE NAME = '홍길동')
AND ORDER_DATE IN (SELECT ORDER_DATE FROM ORDERS WHERE EXTRACT(YEAR FROM ORDER_DATE) = 2024);

-- 3. '유관순' 고객이 지금껏 주문한 상품의 주문번호, 상품명, 수량, 개별금액, 주문별 금액을 조회하시오.
SELECT CUSTOMER_ID FROM CUSTOMERS WHERE NAME = '유관순';

SELECT ORDER_ID 주문번호, PRODUCT_NAME 상품명, QUANTITY 수량, PRICE_PER_UNIT 개별금액, 
(PRICE_PER_UNIT * QUANTITY) 주문별금액합계
FROM ORDER_DETAILS
